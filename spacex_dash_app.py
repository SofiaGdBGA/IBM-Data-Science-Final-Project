...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        # Group the data by LaunchSite and calculate the total number of successful launches
        site_success_counts = successful_launches['Launch Site'].value_counts()
        fig = px.pie(names=site_success_counts.index, values=site_success_counts.values, title='Total Successful Launches by Site',
                    color_discrete_sequence=px.colors.qualitative.Set1)
        return fig
    else:
        filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site]['class'].value_counts()
        fig = px.pie(values=filtered_df.values, names=filtered_df.index, title="Success Rate for {}".format(entered_site), color_discrete_sequence=px.colors.qualitative.Set1)
        return fig
